  ...
  try {
    ...
    const poByJob = await db.query(
      `SELECT job_no,
              ...
              COALESCE(SUM(amount),0)::numeric AS po_value
  ...
  try {
    ...
    const pos = await db.query(
      `SELECT po_no, pr_no, po_type, supplier_name, source_location, status,
              COALESCE(amount,0)::numeric AS amount, delivery_stage, goods_received_date, created_at
    ...
    ok(res, {
      ...
      prs: prs.rows,
      pos: pos.rows.map((p) => ({ ...p, amount: Number(p.amount) })),
    });
  } catch (e) {
    fail(res, 500, e.message);
  }
  ...
  try {
    ...
    const pos = await db.query(
      `SELECT po_no, pr_no, job_no, project_name, po_type, supplier_name, source_location,
              status, COALESCE(amount,0)::numeric AS amount, created_at, goods_received_date
       FROM purchase_orders ORDER BY created_at DESC`,
    );
    // supplier stats: how many POs, how many open, total value
